...
- If both betray: 2 years jail each.
Nash Equilibrium: Both betray (and get a worse outcome than if they cooperated).

## Intuition
Imagine two drivers at an intersection.
- If both go: Crash (Bad).
- If both stop: Wait (Okay).
- If one goes and one stops: Smooth traffic (Good).
Game theory helps predict what will happen.
